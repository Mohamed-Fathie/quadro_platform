...
class WorkshopPopupMenuButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocListener<WorkshopProfileCubit, WorkshopProfileState>(
      listener: (context, state) async {
        if (state.status == WorkshopProfileStatus.editing) {
          final re =
              await NavigationService().routeTo(RoutesConstants.workshopEdit);

          if (re != null) {
            context.read<WorkshopProfileCubit>().refereshe();
          }
        }
      },
      child: PopupMenuButton<WorkshopProfileMenu>(
        onSelected: (value) =>
        ...
        color:
            Qcolors.getCurrentColor(context), // Background color for the menu
        shape: RoundedRectangleBorder(
          borderRadius:
              BorderRadius.circular(8), // Rounded corners for the menu
        ),
        itemBuilder: (context) => [
          PopupMenuItem(
